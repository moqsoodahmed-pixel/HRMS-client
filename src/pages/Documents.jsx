...
const uploadDocument = async (req, res, next) => {
  try {
    ...
    // Wrapped so a storage failure (Cloudinary misconfigured, credentials
    // wrong, package missing, etc.) surfaces its REAL reason to the caller
    // as a proper AppError, instead of falling into errorHandler's generic
    // "An internal server error occurred" catch-all — which hides the
    // underlying message in production and leaves an admin with no way to
    // tell what to fix without reading server logs.
    let filePath;
    try {
      filePath = await storageService_1.storageService.upload(req.file, `documents/${data.employeeId}`);
    } catch (storageErr) {
      console.error('[uploadDocument] storage upload failed:', storageErr);
      throw new errorHandler_1.AppError(
        `Could not save the file: ${storageErr.message}`,
        502,
...
const downloadDocument = async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.error('[downloadDocument] storage download failed:', storageErr);
